...
- processes each request identifier at most once according to its own policy;
- returns a matching acknowledgement or error JSON; and
- responds quickly enough to satisfy the one-second acknowledgement deadline.
...
The app displays a concise status for two seconds and then closes. The
...
| Dictation unavailable | `Dictation unavailable` | Dictation is disabled, cannot be created, or cannot start |
...
| Timeout | `Server timed out` | WebSocket acknowledgement deadline expires |

Exact typography and layout are implementation details, but each message must
fit on `emery` without scrolling. The two-second duration begins when the
...
4. Generate a nonzero request identifier that is unique within the current
   PebbleKit JS connection.
...
If synchronous setup fails before dictation can start, the app enters the
appropriate error state. If asynchronous AppMessage or WebSocket preparation
fails while dictation is active, the app stops and destroys the dictation
...
| `DictationSessionStatusFailureDisabled` | Show dictation-unavailable error |
...
two-second status, and closes. There is no automatic second dictation attempt.
...
| `AWAITING_SEND_START` | Final chunk reached PebbleKit JS; handoff watchdog is active | `AWAITING_SERVER_ACK`, `ERROR` |
...
| `ERROR` | A status is visible for two seconds | closed |
...
for the two-second error duration before the app exits.
...
Successful processing is represented by:
...
the WebSocket and clears the transcript after that AppMessage succeeds or
fails. On any terminal failure it sends `FAILURE` when possible, closes the
WebSocket, and clears all request and transcript state.

## 10. Deadlines

All deadlines use monotonic elapsed time where the runtime provides it.

### 10.1 PebbleKit JS handoff deadline
...
its one-second interval. If the watch's interval expires first, the watch shows
...
After receiving `SEND_STARTED`, the watch starts a two-second bridge-liveness
...
authoritative for whether the server responded within one second. The extra
second allows a response accepted at the end of the server deadline to be
forwarded to the watch without being rejected due only to AppMessage latency.

On PebbleKit JS timeout, it sends `FAILURE` with an acknowledgement-timeout
status. On bridge-liveness expiry without either `REMOTE_ACK` or `FAILURE`, the
watch shows `Delivery failed`, sends best-effort `SESSION_END`, and exits after
the error display.
...
5. displays the mapped status for two seconds; and
6. closes.
...
6. User cancellation, no speech, disabled dictation, and connectivity failure
   each show an appropriate status for two seconds and close without sending a
   WebSocket request.
...
13. A matching acknowledgement received within one second of
    `WebSocket.send()` causes `REMOTE_ACK` and immediate watchapp exit.
...
16. A matching server error produces a two-second delivery error and exit.
17. Lack of `SEND_STARTED` within one second of final-chunk delivery produces a
    two-second delivery error and exit.
18. Lack of a matching server acknowledgement within one second of send
    produces `Server timed out` for two seconds and exit.
19. No timeout or failure path retries dictation, AppMessage transfer,
    WebSocket connection, or WebSocket send.

### 13.4 Cleanup

20. Successful completion leaves no active watch timer or dictation session.
21. Cancellation and every error path release transcript and chunk state.
22. PebbleKit JS closes the request WebSocket and clears volatile request data
    after acknowledgement, failure, or `SESSION_END`.
23. Transcript content is not persisted or logged by watchapp or PebbleKit JS.
